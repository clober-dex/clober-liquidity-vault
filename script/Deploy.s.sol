...
contract DeployScript is Script {
    ...
    // --- Owners (Safe) ---
    address internal constant BASE_SAFE =
        0xfb976Bae0b3Ef71843F1c6c63da7Df2e44B3836d;
    address internal constant MONAD_SAFE =
        0xfb976Bae0b3Ef71843F1c6c63da7Df2e44B3836d;
    address internal constant ARBITRUM_SAFE =
        0xfb976Bae0b3Ef71843F1c6c63da7Df2e44B3836d;

    // --- Core dependencies ---
    address internal constant BASE_BOOK_MANAGER =
        0x8Ca3a6F4a6260661fcb9A25584c796a1Fa380112;
    address internal constant MONAD_BOOK_MANAGER =
    ...
    address internal constant BASE_MINTER_ROUTER =
        0x6131B5fae19EA4f9D964eAc0408E4408b66337b5;
    address internal constant MONAD_MINTER_ROUTER =
        0x7B58A24C5628881a141D630f101Db433D419B372;
    address internal constant ARBITRUM_MINTER_ROUTER =
        0x6131B5fae19EA4f9D964eAc0408E4408b66337b5;

    // --- Oracle parameters ---
    // Chainlink sequencer uptime feeds (L2 only; set to 0 for L1-like chains).
    address internal constant BASE_SEQUENCER_ORACLE =
        0xBCF85224fc0756B9Fa45aA7892530B47e10b6433;
    address internal constant MONAD_SEQUENCER_ORACLE = address(0);
    address internal constant ARBITRUM_SEQUENCER_ORACLE =
    ...
    function deployAll() external broadcast {
        ...
        address strategyProxy = _deploySimpleOracleStrategy(
            cfg,
            oracle,
            vaultProxy
        );
    ...
    function _deployLiquidityVault(
        DeployConfig memory cfg
    ) internal returns (address proxy) {
        // IMPORTANT: Initialize with deployer first, then transfer ownership later.
        address implementation = address(
            new LiquidityVault(IBookManager(cfg.bookManager), BURN_FEE_RATE)
        );
        ...
        LiquidityVault(payable(proxy)).initializeMetadata(
    ...
    function _deploySimpleOracleStrategy(
        DeployConfig memory cfg,
        address oracle,
        ...
        address implementation = address(
            new SimpleOracleStrategy(
                IOracle(oracle),
                ILiquidityVault(payable(liquidityVault)),
                IBookManager(cfg.bookManager)
            )
        );
        console2.log("SimpleOracleStrategy implementation:", implementation);

        bytes memory initData = abi.encodeCall(
            SimpleOracleStrategy.initialize,
            (cfg.deployer)
        );
        proxy = address(new ERC1967Proxy(implementation, initData));
    ...
    function _deployMinter(
        DeployConfig memory cfg,
        address liquidityVault
    ) internal returns (address minter) {
        minter = address(
            new Minter(
                cfg.bookManager,
                payable(liquidityVault),
                cfg.minterRouter
            )
        );
        console2.log("Minter:", minter);
    ...
    function _resolveDeployConfig(
        ...
        if (chainId == ARBITRUM_CHAIN_ID) {
            cfg.owner = ARBITRUM_SAFE;
            cfg.bookManager = ARBITRUM_BOOK_MANAGER;
            cfg.minterRouter = ARBITRUM_MINTER_ROUTER;
            cfg.sequencerOracle = ARBITRUM_SEQUENCER_ORACLE;
            cfg.nativeSymbol = "ETH";
            return cfg;
        }
